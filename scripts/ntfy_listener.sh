...
# Topic name security validation
...
while true; do
    ...
    curl -s --no-buffer "${AUTH_ARGS[@]}" "https://ntfy.sh/$TOPIC/json" 2>/dev/null | while IFS= read -r line; do
        ...
        # Wake shogun via inbox (ntfy messages are received directly by shogun)
        bash "$SCRIPT_DIR/scripts/inbox_write.sh" shogun \
            "New ntfy message received. Check queue/ntfy_inbox.yaml and process it." \
