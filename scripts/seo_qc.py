...
DEFAULT_BASE_DIR = os.environ.get("SEO_AFFILIATE_DIR", os.path.expanduser("~/seo-affiliate"))
...
# Writing style patterns (check_012) - detect jotai (plain form) endings
...
def check_012_writing_style(body: str) -> tuple[bool, str]:
    """Check for consistent desu/masu style (no jotai/plain form endings)."""
    # Exclude HTML tags and frontmatter
    clean = strip_html_tags(body)
    matches = JOTAI_PATTERN.findall(clean)
    if not matches:
        return True, ""
    return False, f"jotai (plain form) found: {matches[:5]}"
...
def print_summary(report: dict):
    ...
    check_names = {
        "check_001": "frontmatter exists",
        "check_002": "frontmatter types",
        "check_003": "PR notation",
        "check_004": "CTA×3",
        "check_005": "CTA structure",
        "check_006": "H2×5",
        "check_007": "FAQ 5 Q&A",
        "check_008": "2500 chars+",
        "check_009": "no forbidden words",
        "check_010": "cost table",
        "check_011": "md syntax",
        "check_012": "desu/masu style",
        "check_013": "area name freq",
        "check_014": "images exist",
    }
