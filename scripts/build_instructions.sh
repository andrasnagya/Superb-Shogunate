...
# ============================================================
# Codex CLI auto-loads AGENTS.md from the repository root.
# Uses CLAUDE.md as the canonical source, replacing Claude-specific parts with Codex equivalents.
generate_agents_md() {
    ...
    sed \
        ...
        -e 's|sends `/clear` + Enter via send-keys|sends `/new` + Enter via send-keys (/clear auto-converted to /new)|g' \
        -e 's|`/clear` sent (max once per 5 min)|skipped (Codex does not support `/clear`)|g' \
        -e 's|escalation sends `/clear` (~4 min)|next nudge escalation or task reassignment|g' \
        -e 's|delivers `/clear` to the agent|delivers `/new` to the agent (/clear auto-converted to /new)|g' \
...
# ============================================================
# GitHub Copilot CLI auto-loads .github/copilot-instructions.md.
# Uses CLAUDE.md as the canonical source, replacing Claude-specific parts with Copilot equivalents.
...
# ============================================================
# Kimi K2 CLI auto-loads agents/default/agent.yaml + system.md.
# Uses CLAUDE.md as the canonical source, replacing Claude-specific parts with Kimi equivalents.
generate_kimi_instructions() {
    ...
    # Generate system.md (CLAUDE.md → Kimi version)
    # Normalize line endings to LF to keep tracked auto-load files stable across platforms.
    sed \
