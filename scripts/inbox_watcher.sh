...
#   Wake-up signal via tmux send-keys (text and Enter sent separately)
#   Agent reads its own inbox and processes messages
#   Idempotent: if delivered twice, nothing happens when no unread messages exist
#
# File changes detected via inotifywait (event-driven, not polling)
# Fallback 1: 30-second timeout (safety net for WSL2 inotify misses)
# Fallback 2: rc=1 handling (Claude Code atomic write = tmp+rename causes inode change)
#
# Escalation (when unread messages are left unprocessed):
#   0-2 min: Standard nudge (send-keys). Skip while Working
#   2-4 min: Escape x2 + nudge (cursor position bug workaround)
#   4 min+:  /clear sent (max once per 5 min. Force reset + YAML re-read)
# ═══════════════════════════════════════════════════════════════
...
if [ "${__INBOX_WATCHER_TESTING__:-}" != "1" ]; then
    ...
    AGENT_ID="$1"
    PANE_TARGET="$2"
    CLI_TYPE="${3:-claude}"  # CLI type (claude/codex/copilot). Unspecified → claude (backward compatible)
...
# Codex has a behavior where input during thinking is immediately picked up, which can cause thought loops. Use a longer cooldown.
...
try:
    ...
    msg = {
        "content": (
            f"[auto-recovery] Post-/clear task resumption notice. "
            f"Re-read queue/tasks/{agent_id}.yaml and resume the assigned task immediately."
        ),
        "from": "inbox_watcher",
...
# CLI_TYPE branching: claude → pass through, codex → /clear handling + skip /model,
#                     copilot → Ctrl-C + restart + skip /model
# At runtime, re-check tmux pane @agent_cli and prefer pane value on drift.
send_cli_command() {
    local cmd="$1"
    local effective_cli
    effective_cli=$(get_effective_cli_type)
    ...
    # Per-CLI command conversion
    local actual_cmd="$cmd"
    case "$effective_cli" in
        codex)
            # Codex: /clear does not exist → use /new to start new conversation, /model unsupported → skip
            # /clear is an undefined command in Codex and would terminate the CLI, so convert to /new
            if [[ "$cmd" == "/clear" ]]; then
            ...
            ;;
        copilot)
            # Copilot: /clear is Ctrl-C + restart, /model unsupported → skip
            if [[ "$cmd" == "/clear" ]]; then
...
agent_is_busy() {
    ...
    effective_cli=$(get_effective_cli_type)
    if [[ "$effective_cli" == "claude" ]]; then
        # Flag file approach: no flag=busy(return 0), flag exists=idle(return 1)
        [ ! -f "${IDLE_FLAG_DIR:-/tmp}/shogun_idle_${AGENT_ID}" ]
    else
        # Legacy pane analysis (Codex etc. fallback)
...
send_wakeup() {
    ...
    # Priority 1: Agent self-watch — no nudge needed (agent notices on its own)
    if agent_has_self_watch; then
        echo "[$(date)] [SKIP] Agent $AGENT_ID has active self-watch, no nudge needed" >&2
        return 0
    fi

    # Priority 2: Agent busy — skip because sending nudge causes Enter to be lost
    ...
    # Priority 3: tmux send-keys (text and Enter separated — Codex TUI workaround)
    ...
    # Clear line (remove residual text) → send nudge → Enter → verify → max 2 retries
    local max_retries=2
    local attempt=0
    while [ $attempt -le $max_retries ]; do
        ...
        timeout 5 tmux send-keys -t "$PANE_TARGET" Enter 2>/dev/null || true
        sleep 0.5
        # Verify delivery: check via capture-pane whether nudge text remains in prompt
        local pane_content
        pane_content=$(timeout 3 tmux capture-pane -t "$PANE_TARGET" -p 2>/dev/null | tail -5 || echo "")
        if echo "$pane_content" | grep -qF "$nudge"; then
            # Nudge text still present → delivery failed → clear with C-u and retry
        ...
        # Delivery succeeded
        # NOTE: Do not delete idle flag. Sending nudge != confirming agent started.
        # Deleting the flag → agent_is_busy()=true → all subsequent nudges skipped → deadlock.
        # Flag naturally disappears when agent actually starts work (consistent with stop_hook design).
...
send_wakeup_with_escape() {
    ...
    # Codex CLI: ESC tends to become "interrupt" and often causes accidents during human operation.
    # Disable Phase 2 Escape escalation, fall back to normal nudge only.
    if [[ "$effective_cli" == "codex" ]]; then
    ...
    # Claude Code: Stop hook detects unread inbox at turn end → auto-processes.
    # Sending Escape is harmful as it interrupts the current turn. Phase 2 falls back to normal nudge.
    if [[ "$effective_cli" == "claude" ]]; then
...
" 2>/dev/null)
    ...
    # /clear is converted to /new on Codex. To prevent message drops right after restart,
    # auto-inject an additional task_assigned to ensure wake-up in the next cycle.
    # Plan B + wait: Allow time for Karo to update task YAML to cancelled before
    # checking status; skip if cancelled/idle.
    # Only trigger auto-recovery on clear_sent (actually sent). Busy-skipped cases are excluded.
    ...
    if [ "$normal_count" -gt 0 ] 2>/dev/null; then
        ...
        if [ "$age" -lt "$ESCALATE_PHASE1" ]; then
            ...
            if [ "$LAST_CLEAR_TS" -lt "$((now - ESCALATE_COOLDOWN))" ]; then
                ...
                effective_cli=$(get_effective_cli_type)
                if [[ "$effective_cli" == "codex" ]]; then
                    # Codex /clear -> /new terminates the conversation, so err on the safe side.
