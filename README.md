...
**Command your AI army like a feudal warlord. Plan like a strategist.**

Run 10 AI coding agents in parallel — **Claude Code, OpenAI Codex, GitHub Copilot, Kimi Code** — orchestrated through a samurai-inspired hierarchy with zero coordination overhead. Includes a vendored [Superpowers](https://github.com/obra/superpowers) planning pipeline and the **soba-yonin** bridge skill for seamless idea-to-execution flow.

**Three approvals from idea to parallel execution.**
...
This project is a fork of [multi-agent-shogun](https://github.com/yohey-w/multi-agent-shogun) by yohey-w — the multi-agent parallel execution system with the Sengoku military hierarchy.

It vendors and modifies [Superpowers](https://github.com/obra/superpowers) by Jesse Vincent (obra) — the planning pipeline (brainstorming, writing-plans, TDD, debugging, and more). Only the `writing-plans` execution handoff is modified to offer the Shogunate as an execution path. Everything else is upstream.
...
### Install & setup:
```bash
claude plugin marketplace add /<PATH-TO-REPO>/Superb-Shogunate
claude plugin install shogunate@shogunate-marketplace
cd ~/.claude/plugins/cache/shogunate-marketplace/shogunate/1.0.0 && sudo bash first_setup.sh
...
### Monitor the agents:
If you really want to closely monitor the agents:
...
soba-yonin handles the rest automatically:
- Shogunate not launched? It calls `shutsujin_departure.sh` for you
- Everything ready? Commands flow straight to the agents

### What is the flow for /superb-plan?
```
/superb-plan
  → brainstorming → design.md
  → writing-plans → implementation.md
  → "Shogunate or Subagent-driven?"
  → soba-yonin delivers → 7 agents execute in parallel
```

### **Want to go deeper?** 
The rest of this README covers architecture, configuration, memory design, and multi-CLI setup.

---

## What is this?

**Superb Shogunate** is a self-contained multi-agent orchestration system that takes ideas from brainstorming through to parallel execution. Based on [multi-agent-shogun](https://github.com/yohey-w/multi-agent-shogun), it bundles the [Superpowers](https://github.com/obra/superpowers) planning pipeline and the **soba-yonin** bridge skill to create a seamless flow from design to deployment.

Supports **Claude Code**, **OpenAI Codex**, **GitHub Copilot**, and **Kimi Code**.

**Why use it?**
- **Three approvals from idea to parallel execution** — approve design, approve plan, pick Shogunate
- One command spawns 7 AI workers + 1 strategist executing in parallel
- Bundled Superpowers planning pipeline (brainstorming + writing-plans) works out of the box
...
## Recommended Workflow: Idea to Parallel Execution
...
│  └── produces: design.md
│
│  writing-plans (bundled, Shogunate-adapted)
│  └── produces: implementation.md
│  └── "Choose: 1) Shogunate  2) Subagent-driven"
│
│  User picks option 1:
│  └── soba-yonin fires automatically
│      ├── Extracts architecture from design.md
│      ├── Splits plan into per-cmd detail files
│      ├── Auto-registers project
│      ├── Writes commands to queue
│      └── Notifies Karo via inbox
│
EXECUTION (tmux, 10 agents)
│
│  Karo decomposes → Ashigaru execute in parallel
│  Gunshi verifies against original design.md
```

**Three approvals:** approve the design, approve the plan, pick Shogunate. That's it.

---

## soba-yonin: The Bridge

The **soba-yonin** skill (`skills/soba-yonin/`) is the integration point between Superpowers planning and Shogun execution. It transforms `design.md` + `implementation.md` into Shogun-protocol YAML commands.

Key features:
- **Heading-agnostic extraction** — reads design.md semantically, works with any spec structure
- **Per-cmd detail files** — pre-splits the implementation plan so ashigaru get exact code and tests
- **Automatic project registration** — derives project ID from the design title
- **Automatic delivery** — writes directly to the Shogun queue and notifies Karo

> *Soba-yonin — chamberlains who whispered in the Shogun's ears.*

---

## Vendored Dependencies

Superb Shogunate bundles [Superpowers v5.0.7](https://github.com/obra/superpowers) at `vendor/superpowers/`. This means the planning pipeline works **out of the box** with zero additional installs.

**Resolution order:** If you have Superpowers installed as a Claude Code plugin, the launcher detects it and prefers the external version (fresher, maintained). If not installed, the bundled version is used automatically.

The launcher logs which version is active at startup:
```
📦 Superpowers: v5.0.7 (external plugin)
```
or:
```
📦 Superpowers: v5.0.7 (bundled) — install Superpowers plugin for latest
```

**What was modified:** Only the `writing-plans` skill's execution handoff section — it offers Shogunate parallel execution as option 1 instead of inline execution. Everything else is unmodified upstream. See `vendor/superpowers/VERSION` for details.

---

## Gunshi's QC: verify-against-design

The `skills/verify-against-design/` skill is Gunshi's core quality assurance tool. It performs **black-box verification** against the original design spec — deriving functional tests from `design.md` alone, never reading the implementation plan.

This catches design-implementation drift that plan-aware testing cannot detect. If something was designed but not implemented, Gunshi finds it.
...
Clone or download and extract to a directory of your choice.

```bash
git clone <repo-url> C:\tools\superb-shogunate
```
...
🖱️ **Run `install.bat`** (Windows only)
...
🐧 **Run `first_setup.sh`** (open Ubuntu / WSL terminal — requires sudo password)

```bash
cd /path/to/superb-shogunate
bash first_setup.sh
```

This installs system packages (tmux, python3), configures MCP, and sets up the queue structure. It requires interactive input and `sudo` — it cannot be run by soba-yonin or Claude Code.
...
This registers skills like `/superb-plan`, `/soba-yonin`, and the forked `/writing-plans` globally with the `(shogunate)` tag.

</td>
</tr>
</table>

**That's the installation.** `shutsujin_departure.sh` (launching agents) is handled automatically by soba-yonin when you use `/superb-plan` — you don't need to run it manually.
...
# 2. OAuth login + Bypass Permissions approval
claude --dangerously-skip-permissions
#    → Browser opens → Log in → "Bypass Permissions" → Select "Yes, I accept"
#    → Type /exit to quit
```
...
2. **Configure SSH** (Settings tab)
   ...
   | Project Path | `/path/to/superb-shogunate` | Server-side project directory |
...
```bash
# 1. Clone
git clone <repo-url> ~/superb-shogunate
cd ~/superb-shogunate

# 2. One-time setup (requires sudo password — run manually)
bash first_setup.sh
...
Use `/superb-plan` in any project. soba-yonin handles `shutsujin_departure.sh` (agent launch) automatically.
...
│                               │                               │
│  ⚔️ Ashigaru 1, leading the charge! │  🔥 Ashigaru 2, pride of the second spear! │
│  Eight blades, one will!            │  Eight blades, one will!                    │
│  ❯                            │  ❯                            │
└───────────────────────────────┴───────────────────────────────┘
```
...
task:
  ...
  echo_message: "🔥 Ashigaru 1, leading the charge! Eight blades, one will!"
```
...
| Karo | Opus | Enabled | Task distribution, QC, dashboard management |
...
```bash
alias csst='cd /path/to/superb-shogunate && ./shutsujin_departure.sh'
