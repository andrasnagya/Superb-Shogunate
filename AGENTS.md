...
- `type: clear_command` → sends `/new` + Enter via send-keys (/clear auto-converted to /new)
...
3. inbox_watcher delivers `/new` to the agent (/clear auto-converted to /new) → session reset
